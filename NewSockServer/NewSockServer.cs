    ...
    public class AsyncTCPServer : IDisposable
    {
        ...
        public void Start()
        {
            if (!IsRunning)
            {
                try
                {
                    ...
                    string msg = DateTime.Now.ToString() + " 服务器(" + this.Address + ":" + this.Port + ")开始监听";
                    ShowMsgEvent(msg);
                    EveryDayLog.Write(msg);
                }
                catch (Exception err)
                {
                    string msg = DateTime.Now.ToString() + " 服务器(" + this.Address + ":" + this.Port + ")监听失败" + err.Message;
                    ShowMsgEvent(msg);
                    EveryDayLog.Write(msg);
                }
        ...
        public void onDataReceived(object sender, AsyncEventArgs e)
        {
            ...
            if (state.clientStatus == 1 )
            {
                ...
                {
                    state.faildTimes++;
                    msg = DateTime.Now.ToString() + " 从待连接客户端(" + ipport + ")非心跳数据:" + sRecvData;
                    ShowMsgEvent(msg);
                    EveryDayLog.Write(msg);
                ...
                if (sOrder.Equals("03"))
                {
                    ...
                    if (sSQL.Substring(sSQL.Length - 2).Equals("),"))
                    {
                        ...
                        try
                        {
                            ...
                            if (iSQLResult > 0)
                            {
                                msg = DateTime.Now.ToString() + " XPH设备(" + comm.serial_num + ")插入最新数据成功:" + sSQL;
                                ShowMsgEvent(msg);
                                EveryDayLog.Write(msg);
                            }
                        }
                        ...
                        {
                            msg = DateTime.Now.ToString() + " XPH设备(" + comm.serial_num + ")插入最新数据失败:" + sSQL;
                            ShowMsgEvent(msg);
                            EveryDayLog.Write(msg);
                        }
                        finally
        ...
        public bool MQTT_ini(string mqtt_host, string mqtt_port)
        {
            ...
            try
            {
                ...
                mqtt_client.Connect(clientId, "kywlw", "12345678");
        ...
        public void getCommand_Thrd()
        {
            while (IsRunning)
            {
                ...
                if (_communicationtype.Contains("2"))
                {
                    ...
                    sSQL += "WHERE a.ActState < 4 AND (ISNULL(a.ScheduledTime) OR (NOW() > a.ScheduledTime AND DATE_ADD(a.ScheduledTime,INTERVAL 10 MINUTE)>NOW())) ";
