...
namespace SocketServer_D
{
    ...
    public class AsyncTCPServer : IDisposable
    {
        ...
        public AsyncTCPServer()
        {
            try
            {
                ...
                mqttserverip = iniFile.IniReadValue("MQTT", "mqtt_host");
                mqttserverport = iniFile.IniReadValue("MQTT", "mqtt_port");
        ...
        public void Start()
        {
            if (!IsRunning)
            {
                try
                {
                    ...
                    if(mqttserverport.Length>0 && mqttserverport.Length>0)
                    {
                        MQTT_Connect();
                    }
        ...
        public void initDev()
        {
            ...
            while (IsRunning)
            {
                ...
                if (_communicationtype.Contains("1"))
                {
                    ...
                    try
                    {
                        //是否有新的通讯设备,通过设备ID，必须自增长，后期可修改为判断id是否重复？
                            ...
                            if (maxCommID > _maxCommID)
                            {
                                ...
                                using (MySqlDataReader msdr2 = myCmd2.ExecuteReader())
                                {
                                    while (msdr2.Read())
                                    {
                                        ...
                                        if(commdev.commtype.Equals("MQT"))
                                        {
                                            MQTT_Listen(commdev.serial_num);
                                        }
                                    }
                                }
                            }
        ...
        public void MQTT_Connect()
        {
            ...
            try
            {
                mqttclient = new MqttClient(mqttserverport, Convert.ToInt32(mqttserverport), false, null, null, MqttSslProtocols.None);
                // 注册接收消息事件 
                mqttclient.MqttMsgPublishReceived += MQT_handlerecv;
                string clientId = Guid.NewGuid().ToString();
                mqttclient.Connect(clientId, "kywlw", "12345678");
                string msg = DateTime.Now.ToString() + " 连接MQTT服务器（" + mqttserverport+":"+ mqttserverport+")成功 ";
                ShowMsgEvent(msg);
                EveryDayLog.Write(msg);
            }
            catch(Exception err)
            {
                string msg = DateTime.Now.ToString() + " 连接MQTT服务器（" + mqttserverport + ":" + mqttserverport + ") 失败，因为 "+err.Message;
                ShowMsgEvent(msg);
                EveryDayLog.Write(msg);
            }
        ...
        public void MQTT_Listen(string commdev_sn)
        {
            string topic = null;
            //订阅主题 "/mqtt/test"， 订阅质量 QoS 2
            topic = "stds/up/CL/" + commdev_sn;
            int msgID = 0;
            msgID = mqttclient.Subscribe(new string[] { topic }, new byte[] { MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE });
            if (msgID > 0)
            {
                string msg = DateTime.Now.ToString() + " MQTT设备（" + commdev_sn + "）订阅成功，消息ID:" + msgID.ToString();
                ShowMsgEvent(msg);
                EveryDayLog.Write(msg);
            }
            //topic = "stds/up/CL/+";
                    ...
                    using (MySqlConnection conn = new MySqlConnection(_connectStr))
                    {
                        ...
                        while (msdr.Read())
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    TCPClientState clientstate = findStatebyCommsn(comm_sn);
                                    if (clientstate != null)
                                    {
                                    ...
                                    {
                                        msg = DateTime.Now.ToString() + " 物联网未找到编号为：" + comm_sn + " 的通讯客户端";
                                        ShowMsgEvent(msg);
                                        EveryDayLog.Write(msg);
                                    }
                                }
                                ...
                                {
                                    msg = DateTime.Now.ToString() + " 通讯设备（" + comm_sn + "）预计时间为空、任务超时或时间未达";
                                    ShowMsgEvent(msg);
                                    EveryDayLog.Write(msg);
                                }
                            }
                        ...
                        while (msdr.Read())
                        {
                            ...
                            try
                            {
                                ...
                                if (schdTime.Length == 0 || (orderOverTime < 600 && orderOverTime >= 0))
                                {
                                    ...
                                    {
                                        msg = DateTime.Now.ToString() + " 水肥机未找到编号为：" + comm_sn + " 的通讯客户端";
                                        ShowMsgEvent(msg);
                                        EveryDayLog.Write(msg);
                                    }
                                }
                                ...
                                {
                                    msg = DateTime.Now.ToString() + " 水肥机（" + comm_sn + "）没有预计时间、任务超时或时间未达";
                                    ShowMsgEvent(msg);
                                    EveryDayLog.Write(msg);
                                }
                            }
        ...
        public object findDevbyAddr(int for_value, string comm_sn, int devclass)
        {
            ...
            {
                string msg = DateTime.Now.ToString() + " 通过地址（地址+通讯设备序列号+设备类别:" + for_value.ToString()+ ":"+ comm_sn + ":" + devclass.ToString() + ")获取设备发生错误:" + err.Message; ;
                ShowMsgEvent(msg);
                EveryDayLog.Write(msg);
                return sReturn;
            }
        ...
        public object findDevbyID(int for_value, int devclass)
        {
            ...
            {
                string msg = DateTime.Now.ToString() + " 通过ID（id+设备类别:" + for_value.ToString() + ":" + devclass.ToString() + ")获取设备发生错误:" + err.Message; ;
                ShowMsgEvent(msg);
                EveryDayLog.Write(msg);
                return sReturn;
            }
        ...
        public TCPClientState findStatebyCommsn(string commsn)
        {
            TCPClientState result=null;
            try
            {
                if (commsn.Length > 0 && _clients.Count > 0)
                {
                    int i = 0;
                    while (i < _clients.Count)
                    {
                        if(_clients[i].commsn.Equals(commsn) && _clients[i].clientStatus==2)
                        {
                            return _clients[i];
                        }
                        i++;
            ...
            {
                string msg = DateTime.Now.ToString() + " 判断通讯设备:" + commsn + " 是否有通讯客户端中发生:" + err.Message; ;
                ShowMsgEvent(msg);
                EveryDayLog.Write(msg);
                return result;
            }
    ...
    public class TCPClientState
    {
        ...
        public string commsn
        {
            get
            {
                if (_clientComm != null )
                {
                    return _clientComm.serial_num;
                }
                else
                {
                    return "";
